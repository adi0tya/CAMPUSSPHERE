...
import { HiShieldCheck, HiEnvelope, HiLockClosed, HiEye, HiEyeSlash, HiUser, HiPhone, HiKey } from 'react-icons/hi2';
...
const AdminLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true);
    try { await adminLogin(loginForm.email, loginForm.password); navigate('/admin/dashboard', { replace: true }); }
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    if (!regForm.name || !regForm.email || !regForm.phone || !regForm.password || !regForm.confirmPassword || !regForm.secretCode) return toast.error('Fill all fields');
    ...
    setLoading(true);
    try { await adminRegister(regForm); navigate('/admin/dashboard', { replace: true }); }
    catch (err) { toast.error(err.response?.data?.message || 'Registration failed'); }
    finally { setLoading(false); }
  };

  const inputClass = "w-full pl-11 pr-4 py-3 bg-black border border-[#2a2a2a] rounded-xl text-white placeholder-gray-600 text-sm focus:border-cyan-500";
    ...
    <div className="min-h-screen bg-black flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-xl gradient-cyan mb-4 shadow-lg shadow-cyan-500/20">
            <HiShieldCheck className="w-8 h-8 text-white" />
          </div>
          <h1 className="text-3xl font-bold text-white">Admin Portal</h1>
          <p className="text-gray-500 mt-2">Sign in or create your admin account</p>
        </div>

        {/* Tabs */}
        <div className="flex bg-[#111111] rounded-xl p-1 mb-6 border border-[#2a2a2a]">
          <button onClick={() => setTab('login')} className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all ${tab === 'login' ? 'bg-cyan-500 text-white' : 'text-gray-500 hover:text-white'}`}>Login</button>
          <button onClick={() => setTab('register')} className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all ${tab === 'register' ? 'bg-cyan-500 text-white' : 'text-gray-500 hover:text-white'}`}>Register</button>
        </div>
        ...
        {tab === 'login' ? (
          <form onSubmit={handleLogin} className="glass p-8 space-y-5">
            <div><label className="block text-sm font-medium text-gray-400 mb-2">Email</label>
              <div className="relative"><HiEnvelope className={iconClass} /><input type="email" value={loginForm.email} onChange={e => setLoginForm({...loginForm, email: e.target.value})} placeholder="admin@example.com" className={inputClass} /></div></div>
            ...
            <button type="submit" disabled={loading} className="w-full py-3 gradient-cyan rounded-xl text-white font-semibold hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2">
          ...
          <form onSubmit={handleRegister} className="glass p-8 space-y-4">
            ...
            <div><label className="block text-sm font-medium text-gray-400 mb-1.5">Email</label>
              <div className="relative"><HiEnvelope className={iconClass} /><input type="email" value={regForm.email} onChange={e => setRegForm({...regForm, email: e.target.value})} placeholder="admin@example.com" className={inputClass} /></div></div>
            ...
            <div><label className="block text-sm font-medium text-gray-400 mb-1.5">Admin Secret Code</label>
              <div className="relative"><HiKey className={iconClass} /><input type="password" value={regForm.secretCode} onChange={e => setRegForm({...regForm, secretCode: e.target.value})} placeholder="Enter admin secret code" className={inputClass} /></div>
              <p className="text-xs text-gray-600 mt-1">Contact system administrator for the code</p></div>
            <button type="submit" disabled={loading} className="w-full py-3 gradient-cyan rounded-xl text-white font-semibold hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2">
