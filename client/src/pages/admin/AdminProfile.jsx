  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="glass p-6 flex items-center gap-4">
        <div className="w-16 h-16 rounded-2xl gradient-cyan flex items-center justify-center text-white text-2xl font-bold">{user?.name?.charAt(0)}</div>
        <div><p className="text-xl font-bold text-white">{user?.name}</p><p className="text-gray-500">{user?.email}</p><span className="px-2.5 py-1 mt-1 inline-block rounded-full text-xs font-medium bg-cyan-500/10 text-cyan-400 capitalize">{user?.role}</span></div>
      </div>
      ...
      <form onSubmit={handlePassword} className="glass p-6 space-y-4">
        ...
        <button type="submit" className="px-6 py-2.5 bg-[#181818] border border-[#2a2a2a] rounded-xl text-white font-semibold hover:border-[#333]">Update Password</button>
      </form>
    </div>
  );
};
