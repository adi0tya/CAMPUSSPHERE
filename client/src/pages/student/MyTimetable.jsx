...
const MyTimetable = () => {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold text-white">My Timetable</h1><p className="text-gray-600 text-sm">Your weekly class schedule</p></div>
      {timetable.length === 0 ? <EmptyState title="No timetable available" icon={HiCalendarDays} /> : (
        <div className="space-y-4">
          {DAYS.map(day => grouped[day].length > 0 && (
            <div key={day} className={`glass overflow-hidden ${day === today ? 'border-cyan-500/30' : ''}`}>
              <div className="px-5 py-3 border-b border-[#2a2a2a] flex items-center gap-2">
                ...
                {day === today && <span className="px-2 py-0.5 rounded-full text-xs bg-cyan-500/10 text-cyan-400">Today</span>}
              </div>
                ...
                {grouped[day].map(t => (
                  <div key={t._id} className="px-5 py-4 flex items-center gap-4 hover:bg-[#0a0a0a]">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">{t.faculty?.user?.name} • {t.room || 'No room'}</p>
                    </div>
                  </div>
                ))}
              </div>
